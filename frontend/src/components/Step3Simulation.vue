...
const doStartSimulation = async () => {
  ...
  // Reset all state to ensure clean start from previous simulation
  
  isStarting.value = true
  startError.value = null
  addLog('Launching dual-platform parallel simulation...')
...
// Track each platform's last round for detecting changes and logging
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Check if simulation is complete (via runner_status or platform completion state)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Additional check: if backend hasn't updated runner_status yet but platforms report complete
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform is complete, check if all enabled platforms are done
  // Determine if platform is enabled by actions_count (> 0 or running was true)
  ...
  // Check that all enabled platforms are complete
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // No auto-scroll; let user freely browse the timeline
